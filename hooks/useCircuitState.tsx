...
import { Gate, GateType, Qubit, Circuit, SimulationResults, Complex } from '@/lib/types';

// Mock simulation function - in a real app, this would use a quantum simulation library
const mockSimulation = (circuit: Circuit, qubits: Qubit[]): SimulationResults => {
  ...
  const totalProb = stateVector.reduce((sum, state) => sum + Math.pow(state.magnitude, 2), 0);
  ...
  const qubitStates = qubits.map((_, i) => {
    return {
      stateVector: [
        ...
        { re: 0, im: Math.random(), magnitude: Math.random(), phase: Math.PI / 2 }
      ],
...
export function useCircuitState() {
    ...
    if (qubits.length > 1) {
      ...
      setCircuit(prev => ({
        ...
        gates: prev.gates.filter(gate => 
          !gate.qubitIndices.includes(lastQubitIndex)
        )
      }));
  ...
  const updateGateParameters = useCallback((gateId: string, parameters: Gate['parameters']) => {
    setCircuit(prev => ({
      ...prev,
      gates: prev.gates.map(gate => 
        gate.id === gateId 
  ...
  const generateQASM = useCallback(() => {
    ...
    const gateInstructions = sortedGates.map(gate => {
      switch (gate.type) {
        case 'h':
    ...
    const measureInstructions = qubits.map((_, i) => `measure q[${i}] -> c[${i}];`).join('\n');
